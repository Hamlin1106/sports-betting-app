...
const SubmitButton: React.FC = () => {
  ...
  console.log("submitbutton component rendered");
  ...
  const {
    ...
    onSuccess: () => {
      clear()
      console.log("Bet submitted succefully!")
    },
    onError: (error) => {
      console.log("Error during bet submission:", error)
    }
  })
  ...
  return (
    <div className="mt-6">
      ...
      <button
        ...
        disabled={isPending || isProcessing || isApproveRequired}
        onClick={() => { alert(); console.log("Place bet button is clicked!") }}
      >
        {isPending ? 'Pending...' : isProcessing ? 'Processing...' : 'Place Bet'}
      </button>
    </div>
  )
}
